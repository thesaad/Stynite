       ...
       <div class="pageicon"><span class="iconfa-list"></span></div>
        ...
        <div class="maincontent">
            <div class="maincontentinner">
                    ...
                    <h4 class="widgettitle">All Retailers
                  	
<a style="float: right; margin-top: -8px;" onclick="retailer_action('delete')" title="Delete Selected Retailer" class="btn btn-danger btn-rounded"><i class="iconfa-trash"></i>&nbsp;Delete Selected</a>
&nbsp;&nbsp;
                    	<a onclick="retailer_action('active')" style="float: right; margin-top: -8px;"  title="Active Retailer" class="btn btn-success btn-rounded">&nbsp;Active&nbsp;&nbsp;</a>&nbsp;&nbsp;
<a onclick="retailer_action('detactive')" style="float: right; margin-top: -8px;"  title="Deactive Retailer" class="btn btn-warning btn-rounded">&nbsp;Deactive&nbsp;&nbsp;</a>&nbsp;&nbsp;
             ...
             <table id="employee-grid"  class="table table-bordered table-infinite" cellpadding="0" cellspacing="0" border="0" class="display" width="100%">
					<thead>
						<tr>
							...
							<th style="cursor: pointer">Contact</th>
						 <th style="cursor: pointer">Email</th>
							
							<th style="cursor: pointer">View Products</th>
							<th style="cursor: pointer">Stats</th>
							<th style="cursor: pointer">Sales</th>
            ...
            <form  id="retaileraction"  method="POST"  >
...
function initPOPOVER()
{
    ...
    function retailer_action(action)
{
	$("#action").val(action);
  if(confirm("Are you sure want to "+action+" ? "))
  {
       var formData = new FormData($('#retaileraction')[0]);
                            $.ajax({
                                
              		    type: "post",
             			url : "<?php echo site_url("admin/retaileraction");?>",
               			data: formData,
                        contentType: false,
                        processData: false,
               			success:	function(data){
                     			if(data==1){
                                   alert("Retailers "+action+" successfully.");
		...
		<script type="text/javascript" language="javascript" >
			$(document).ready(function() {
				var dataTable = $('#employee-grid').DataTable( {
					...
					"order": [[ 4, "desc" ]],
					"ajax":{
						url :'<?=site_url('admin/get_retailers_dt');?>', // json datasource
