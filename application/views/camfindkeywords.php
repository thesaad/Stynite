...
<div class="rightpanel">
       ...
       <div class="pageicon"><span class="iconfa-camera"></span></div>
            <div class="pagetitle">
                <h1>Camfind Keywords</h1>
            </div>
        </div><!--pageheader-->
        <div class="maincontent">
            <div class="maincontentinner">
                    
                    <h4 class="widgettitle"> Camfind Keywords
               	
<a style="float: right; margin-top: -8px;" onclick="select_keyword()" title="Custom Keyword" class="btn btn-danger btn-rounded"><i class="iconfa-tool"></i>&nbsp;Custom keyword</a>
 
 <a href="<?php echo site_url("admin/camfindkeywords_manage_keyword");?>" style="float: right; margin-top: -8px;"  title="Add" class="btn btn-success btn-rounded"><i class="iconfa-plus"></i>&nbsp;Add</a>
             ...
             <table id="employee-grid"  class="table table-bordered table-infinite" cellpadding="0" cellspacing="0" border="0" class="display" width="100%">
					<thead>
						<tr>
							<th style="width:50px; cursor: pointer" >Check</th>
							<th style="cursor: pointer">Keyword</th>
							 <th style="cursor: pointer">Custom Keyword</th>
							 <th style="cursor: pointer">Affilate Keyword</th>
							 <th style="cursor: pointer">Stynite Keyword</th>
							<th>Image</th>
							<th>Action</th>
							<th>Select</th>
						</tr>
            ...
            <form  id="keywordaction"  method="POST"  >
...
function initPOPOVER()
{
    ...
    function keyword_action(action)
{
	$("#action").val(action);
  if(confirm("Are you sure want to "+action+" ? "))
  {
       var formData = new FormData($('#keywordaction')[0]);
                            $.ajax({
                                
              		    type: "post",
             			url : "<?php echo site_url("admin/camkeywordaction");?>",
               			data: formData,
                        contentType: false,
                        processData: false,
               			success:	function(data){
                     			if(data==1){
                                   alert("Keywords "+action+" successfully.");
  ...
  if(confirm("Are you sure want to delete? "))
  {
       var formData = new FormData($('#keywordaction')[0]);
             			...
             			url : "<?php echo site_url("admin/camkeywordaction");?>",
               			...
               			success:	function(data){
                     			if(data==1){
                                   alert("Keywords deleted successfully.");
                                   window.location.reload();
                				}else{
                                   alert("Try again!! Error occur while delete."); 
                                    }
		...
		<script type="text/javascript" language="javascript" >
            ...
            },
            {
                ...
                "visible": false
					...
					"ajax":{
						url :'<?=site_url('admin/get_camfindkeywords_dt');?>', // json datasource
				...
				$('#employee-grid').on('search.dt', function() {
    var value = $('.dataTables_filter input').val();
    console.log(value); // <-- the value
     var column = dataTable.column(0);
  var column2 = dataTable.column(7);
...
{
  if(confirm("Are you sure you want to merge custom keywords for the checked?")){
  	
  	 
  	 var formData = new FormData($('#keywordaction')[0]);
  	$.ajax({
				        url         :'<?=site_url('admin/merge_keywords');?>',
				        data        :formData,
				        type        :'POST',
				          contentType: false,
                        processData: false,
