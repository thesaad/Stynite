...
<div class="rightpanel">
       ...
       <div class="pageicon"><span class="iconfa-list"></span></div>
            <div class="pagetitle">
                <h1> <span style="color: red"><?php echo $user[0]['firstname']." ".$user[0]['lastname'];?> </span></h1>
                <a href="mailto:<?php echo $user[0]['email']?>"><?php echo $user[0]['email']?></a>
            </div>
        </div><!--pageheader-->
        <div class="maincontent">
            <div class="maincontentinner">
                    <?php $this->view("product_notice");?>
                    <h4 class="widgettitle">Affiliate Product list
 <!--
 <a onclick="recommendation_action('1')" style="float: right; margin-top: -8px;"  title="Active Retailer" class="btn btn-success btn-rounded">&nbsp;Active&nbsp;&nbsp;</a>&nbsp;&nbsp;
<a onclick="recommendation_action('2')" style="float: right; margin-top: -8px;"  title="Deactive Retailer" class="btn btn-warning btn-rounded">&nbsp;Deactive&nbsp;&nbsp;</a>
 -->
                    </h4>
             <table id="employee-grid"  class="table table-bordered table-infinite" cellpadding="0" cellspacing="0" border="0" class="display" width="100%">
					<thead>
						<tr>
							...
							<th style="cursor: pointer">Store</th>
						...
						<!--<th style="cursor: pointer">Recommendation</th>
						<th style="cursor: pointer">Select</th>-->
							
						</tr>
					</thead>
			</table>
           <form  id="recommendationproduct"  method="POST"  >
...
function initPOPOVER()
{
    ...
    function recommendation_action(action)
{
	if(action=='1'){
		choice = "on";
	}else{
	...
	$("#action").val(action);
  if(confirm("Are you sure want to "+choice+" ? "))
  {
       var formData = new FormData($('#recommendationproduct')[0]);
                            $.ajax({
                                
              		    type: "post",
             			url : "<?php echo site_url("user/affilate_recommendation_action");?>",
               			data: formData,
                        contentType: false,
                        processData: false,
               			success:	function(data){
                     			if(data==1){
                                   alert("Recommendation "+choice+" successfully.");
                                   window.location.reload();
                				}else{
                                   alert("Try again!! Error occur."); 
                                    }
                			}
		...
		<script type="text/javascript" language="javascript" >
			$(document).ready(function() {
				var dataTable = $('#employee-grid').DataTable( {
					...
					"order": [[ 8, "desc" ]],
					"ajax":{
						url :'<?=site_url('user/get_useraffilateproducts_dt')."?id=".$_GET['id'];?>', // json datasource
