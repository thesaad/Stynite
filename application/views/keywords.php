...
<a style="float: right; margin-top: -8px;" onclick="keyword_action('delete')" title="Delete Selected Retailer" class="btn btn-danger btn-rounded"><i class="iconfa-trash"></i>&nbsp;Delete Selected</a>
 ...
 <a href="<?php echo site_url("admin/manage_keyword");?>" style="float: right; margin-top: -8px;"  title="Add" class="btn btn-success btn-rounded"><i class="iconfa-plus"></i>&nbsp;Add</a>
    ...
    function keyword_action(action)
{
  ...
  if(confirm("Are you sure want to "+action+" ? "))
  {
       var formData = new FormData($('#keywordaction')[0]);
              		    ...
              		    type: "post",
             			url : "<?php echo site_url("admin/keywordaction");?>",
               			data: formData,
                        contentType: false,
                        processData: false,
		...
		<script type="text/javascript" language="javascript" >
			$(document).ready(function() {
				var dataTable = $('#employee-grid').DataTable( {
					...
					"order": [[ 1, "asc" ]],
					"ajax":{
						url :'<?=site_url('admin/get_keywords_dt');?>', // json datasource
